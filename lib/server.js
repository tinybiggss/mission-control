...
// src/drift.js
var require_drift = __commonJS({
  "src/drift.js"(exports2, module2) {
    var fs2 = require("fs");
    var path2 = require("path");
    var { cleanTaskText, readLatestTaskData } = require_focus();
    var { buildCaptureEntry, appendCapture } = require_dispatch();
    var VAULT_ROOT = "/Users/michaeljones/Dev/Obsidian/Mike_Thinking_Space";
    var DAILY_NOTES_DIR = path2.join(VAULT_ROOT, "Week and Daily Plans");
    var TASK_DATA_DIR = path2.join(VAULT_ROOT, "Corvus", "Task Data");
    var QUEUE_FILE = "/Users/michaeljones/.openclaw/workspace/Corvus/Operations/capture-queue.json";
    var DRIFT_MIN = 10;
    var TITLE_KEY_LEN = 40;
    var ACTIONS = ["set-due", "delegate", "drop"];
    function titleKeyFor(taskText) {
      return cleanTaskText(taskText).toLowerCase().slice(0, TITLE_KEY_LEN);
    }
    var OPEN_CHECKBOX_RE = /^\s*- \[ \]/;
    ...
    var DUE_RE = /📅\s*\d{4}-\d{2}-\d{2}/u;
    function applyAction(line, action, opts = {}) {
      if (action === "set-due") {
        if (DUE_RE.test(line)) return line.replace(DUE_RE, `\u{1F4C5} ${opts.due}`);
        const m = line.match(/\s(\[↩|#)/u);
        if (m) return line.slice(0, m.index) + ` \u{1F4C5} ${opts.due}` + line.slice(m.index);
        return `${line} \u{1F4C5} ${opts.due}`;
      }
      if (action === "delegate") {
    ...
    function newestDailyNote(dir) {
      try {
        const files = fs2.readdirSync(dir).filter((f) => /^\d{4}-\d{2}-\d{2}\.md$/.test(f)).sort();
        return files.length ? path2.join(dir, files[files.length - 1]) : null;
      } catch {
        return null;
      }
    }
    ...
    function runDriftAction(deps, { task, action, due }) {
      ...
      const content2 = fs2.readFileSync(noteFile, "utf8");
      const found = findTaskLine(content2, titleKeyFor(task));
      if (found.error) {
        return {
          ok: false,
          status: found.error === "ambiguous" ? 409 : 404,
          error: found.error === "ambiguous" ? "matches more than one open task line \u2014 edit in Obsidian" : "task line not found in the newest daily note \u2014 it may have moved"
        };
      }
      ...
      fs2.writeFileSync(tmp, lines.join("\n"));
      fs2.renameSync(tmp, noteFile);
      if (action === "delegate") {
        appendCapture(
          queueFile,
          buildCaptureEntry({
    ...
    function createDriftAPI2(deps = {}) {
      const readTasks = deps.readTasks || (() => readLatestTaskData(TASK_DATA_DIR));
      return {
        list(req, res) {
          const tasks = listDrifting(readTasks);
          res.writeHead(200, { "Content-Type": "application/json" });
          res.end(JSON.stringify({ tasks, driftMin: DRIFT_MIN, asOf: (/* @__PURE__ */ new Date()).toISOString() }, null, 2));
        },
        /**
    ...
    module2.exports = {
      ...
      listDrifting,
      runDriftAction,
      createDriftAPI: createDriftAPI2
    };
  }
});

// src/overnight.js
var require_overnight = __commonJS({
  "src/overnight.js"(exports2, module2) {
    var fs2 = require("fs");
    var path2 = require("path");
    var { readLatestTaskData, cleanTaskText } = require_focus();
    var CRON_JOBS_FILE = "/Users/michaeljones/.openclaw/cron/jobs.json";
    var CRON_STATE_FILE = "/Users/michaeljones/.openclaw/cron/jobs-state.json";
    var QUEUE_FILE = "/Users/michaeljones/.openclaw/workspace/Corvus/Operations/capture-queue.json";
    var TASK_DATA_DIR = path2.join(
      "/Users/michaeljones/Dev/Obsidian/Mike_Thinking_Space",
      "Corvus",
      "Task Data"
    );
    var WINDOW_MS = 12 * 3600 * 1e3;
    var DRIFT_MIN = 10;
    var cache = { data: null, timestamp: 0, refreshing: false };
    var TTL_MS = 3e5;
    ...
    function dreamingLine(getMind) {
      try {
        ...
        if (!mind || mind.mounted === false) {
          return "\u{1F319} Memory ledger offline \u2014 no consolidation to report.";
        }
        const c = mind.consolidation;
        if (!c) return "\u{1F319} No dreaming consolidation found (runs 3:00 AM).";
        const bits = [];
        ...
        return `\u{1F319} Dreaming ran (${c.date}) \u2014 ${bits.join(" \xB7 ") || "empty consolidation"}.`;
      } catch {
        return null;
      }
    }
    function cronLine(readJobs, readJobsState, now) {
      try {
        const jobs = readJobs();
        const state2 = readJobsState();
        if (!jobs || !state2 || !state2.jobs) return null;
        const nameById = {};
        for (const j of jobs.jobs || []) nameById[j.id] = j.name || j.id;
        let ran = 0;
        const failed = [];
        for (const [id, entry] of Object.entries(state2.jobs)) {
        ...
        return failed.length ? `\u2699\uFE0F ${ran} cron runs in the last 12h \u2014 ${failed.length} failing: ${failed.join(", ")}.` : `\u2699\uFE0F ${ran} cron runs in the last 12h, all ok.`;
      } catch {
        return null;
      }
    }
    function ledgerLine(getMind) {
      try {
        ...
        const c = mind.counts;
        return `\u{1F9E0} Ledger: ${c.sessions || 0} session summaries \xB7 ${c.consolidations || 0} consolidations${c.stubs ? ` \xB7 ${c.stubs} unfilled stubs` : ""}.`;
      } catch {
        return null;
      }
    }
    function queueLine(queueFile) {
      ...
      return `\u{1F4E5} Corvus inbox: ${q.length} item${q.length === 1 ? "" : "s"} queued for the next heartbeat.`;
    }
    function driftLineAndCount(readTasks) {
      try {
        ...
        return {
          line: `\u{1F3AF} ${drifting} task${drifting === 1 ? "" : "s"} drifting at \u21A910+ \u2014 triage in the Drift Meter.`,
          drifting
        };
      } catch {
    ...
    function computeOvernight(deps = {}) {
      const getMind = deps.getMind || require_system_mind().getSystemMindCached;
      ...
      const drift2 = driftLineAndCount(readTasks);
      const lines = [
      ...
      return {
        lines,
        stats: { drifting: drift2.drifting },
        asOf: (/* @__PURE__ */ new Date()).toISOString()
      };
    }
    ...
    function createOvernightAPI2(deps = {}) {
    ...
    module2.exports = { computeOvernight, createOvernightAPI: createOvernightAPI2, getOvernightCached };
  }
});

// src/ops-registry.js
var require_ops_registry = __commonJS({
  "src/ops-registry.js"(exports2, module2) {
    var fs2 = require("fs");
    var path2 = require("path");
    var os = require("os");
    var { execFileSync } = require("child_process");
    var { cronToHuman } = require_cron();
    var OPENCLAW_CRON_DIR = path2.join(os.homedir(), ".openclaw", "cron");
    var LAUNCH_AGENTS_DIR = path2.join(os.homedir(), "Library", "LaunchAgents");
    var RELEVANT_PLIST_RE = /^(ai\.openclaw|com\.mike|local\.mike|launchagent-)/;
    var PARKED_LABELS = /* @__PURE__ */ new Set(["local.mike.corvus-scheduler", "local.mike.corvus-publisher"]);
    var RECENT_WINDOW_MS = 12 * 3600 * 1e3;
    var MAX_UPCOMING = 8;
    var cache = { data: null, timestamp: 0, refreshing: false };
    var TTL_MS = 6e4;
    ...
    function plistSchedule(plist) {
      ...
      if (cal) {
        ...
        const weekdays = entries.map((e) => e.Weekday).filter((w) => w != null).map((w) => WEEKDAY_NAMES[w] || w);
    ...
    function parseCrontab(text) {
      ...
      for (const line of String(text || "").split("\n")) {
        ...
        items.push({
          ...
          status: "ok",
          // crontab keeps no run history; presence = scheduled
    ...
    function shapeOpenclawCron(job, stateEntry) {
      ...
      if (job.enabled === false) {
        ...
        status = "failing";
        detail = s.consecutiveErrors ? `${s.consecutiveErrors} consecutive error${s.consecutiveErrors === 1 ? "" : "s"}` : `last run: ${s.lastRunStatus}`;
    ...
    function timeline(items, now) {
      const recent = items.filter((i) => i.lastRunAt && now - i.lastRunAt <= RECENT_WINDOW_MS).sort((a, b) => b.lastRunAt - a.lastRunAt).map((i) => ({ name: i.name, at: i.lastRunAt, status: i.status }));
      const upcoming = items.filter((i) => i.nextRunAt && i.nextRunAt > now && i.status !== "disabled").sort((a, b) => a.nextRunAt - b.nextRunAt).slice(0, MAX_UPCOMING).map((i) => ({ name: i.name, at: i.nextRunAt }));
      return { recent, upcoming };
    }
    function safeExec(cmd, args2) {
      try {
        return execFileSync(cmd, args2, { timeout: 5e3, encoding: "utf8" });
      } catch {
        return null;
      }
    }
    ...
    function defaultListPlists() {
      try {
        return fs2.readdirSync(LAUNCH_AGENTS_DIR).filter((f) => f.endsWith(".plist") && RELEVANT_PLIST_RE.test(f)).map((f) => {
          const raw = safeExec("plutil", ["-convert", "json", "-o", "-", path2.join(LAUNCH_AGENTS_DIR, f)]);
          let plist = null;
          try {
    ...
    function computeOps(deps = {}) {
      const readCronJobs = deps.readCronJobs || (() => readJsonOrNull(path2.join(OPENCLAW_CRON_DIR, "jobs.json")));
      const readCronState = deps.readCronState || (() => readJsonOrNull(path2.join(OPENCLAW_CRON_DIR, "jobs-state.json")));
      ...
      try {
        ...
        const state2 = readCronState();
        if (!jobs) throw new Error("jobs.json unreadable");
        openclawCron = (jobs.jobs || []).map((j) => shapeOpenclawCron(j, state2 && state2.jobs && state2.jobs[j.id]));
      } catch (e) {
      ...
      try {
        ...
        launchAgents = [...byLabel.entries()].map(([label, plists]) => {
          ...
          if (plists.length > 1) {
            item.detail = `${item.detail ? item.detail + " \xB7 " : ""}\u26A0 ${plists.length} plist files share this label \u2014 remove the duplicate`;
          }
          return item;
        });
      } catch (e) {
...
var { createDriftAPI } = require_drift();
var { createOvernightAPI } = require_overnight();
var { createOpsAPI } = require_ops_registry();
...
var drift = createDriftAPI({});
var overnight = createOvernightAPI({});
var ops = createOpsAPI({});
