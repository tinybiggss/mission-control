...
var require_health = __commonJS({
  "src/health.js"(exports2, module2) {
    ...
    function checkCurlJob(id, label, filePath) {
      ...
      let raw = "";
      try {
        raw = fs2.readFileSync(filePath, "utf8").trim();
      } catch {
      }
      ...
      const success = parsed && (parsed.ok === true || parsed.success === true || parsed.status === "ok");
      return {
        id,
        label,
        ok: !!success,
        severity: "warn",
        value: relAge(st.mtimeMs),
        detail: success ? `Last run OK ${relAge(st.mtimeMs)}.` : `Last run reported not-ok ${relAge(st.mtimeMs)}.`
      };
    }
